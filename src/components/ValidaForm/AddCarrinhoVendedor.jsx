  ...
  const handleListarProdutosCarrinho = () => {
    $.ajax({
      ...
      error: function (xhr, status, error) {
        show("Erro ao carregar produtos do carrinho:", "error");
      },
    });
  };
  ...
  const handleEnviarComprovativo = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Aqui você pode fazer alguma ação após a compra bem-sucedida
   
      } else {
        show( response.data.message,"erro");
      }
    } catch (error) {
      console.error("Erro ao efetuar compra:", error);
      alert("Erro ao efetuar compra. Verifique o console para mais detalhes.");
    }
  };
