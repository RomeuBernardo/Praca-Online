...
function Admin() {
  ...
  const [totalClientes, setTotalClientes] = useState(0); // Estado para armazenar o total de clientes
  ...
  const handleListarClientes = () => {
    $.ajax({
      url: "http://localhost:5000/ListarClientes",
      type: "GET",
      dataType: "json",
      success: function (response) {
        if (response.success) {
          ...
          setData(clientes);
          setTotalClientes(clientes.length); // Atualizar o total de clientes
        } else {
  ...
  const handleEliminar = (id_cliente) => {
    $.ajax({
      url: "http://localhost:5000/removerCliente",
      type: "POST",
  ...
  //Funcao para Editar clientes
  const handleEditarCliente = () => {
    $.ajax({
      url: "http://localhost:5000/editarCliente",
      type: "POST",
      contentType: "application/json",
      ...
      success: function (response) {
        if (response.success) {
         show("Cliente editado com sucesso!","success");
          handleListarClientes(); // Atualizar a lista de clientes após a edição
  ...
  const handleDesabilitar = (item) => {
    // Implemente a lógica para desabilitar o cliente aqui
    console.log("Cliente desabilitado:", item);
  };
      ...
      <h1>Lista  de Clientes</h1>
        ...
        <table className="p-table">
          ...
          <tbody>
              ...
              <tr key={item.id_cliente}>
                ...
                <td>
                  <div className="button-group">
                    <Button
                      label="Remover"
                      onClick={() => handleEliminar(item.id_cliente)}
                      icon="pi pi-trash"
            ...
            <tr>
              <td colSpan="6"></td>
              <td>Total de Clientes: {totalClientes}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <Dialog
  header={<strong>Editar Cliente</strong>}
  visible={visible}
  style={{
    width: "50vw",
